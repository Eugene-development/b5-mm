...
<div
	class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4 py-8 sm:py-16"
>
	<!-- Animated background elements -->
	<div class="pointer-events-none fixed inset-0 overflow-hidden">
		<div
			class="animate-blob absolute -left-32 top-0 h-96 w-96 rounded-full bg-purple-600/20 blur-3xl"
		></div>
		<div
			class="animate-blob animation-delay-2000 absolute -right-32 top-1/3 h-96 w-96 rounded-full bg-blue-600/20 blur-3xl"
		></div>
		<div
			class="animate-blob animation-delay-4000 absolute -bottom-32 left-1/3 h-96 w-96 rounded-full bg-indigo-600/20 blur-3xl"
		></div>
	</div>
	...
	<div class="relative z-10 mx-auto max-w-xl">
		...
		{#if !successMessage}
			<div class="mb-8 text-center">
				<h1
					class="bg-gradient-to-r from-white via-purple-200 to-blue-200 bg-clip-text text-3xl font-bold text-transparent sm:text-4xl"
				>
					Создать проект
				</h1>
		...
		<div class="group relative">
			<!-- Glow effect -->
			<div
				class="absolute -inset-0.5 rounded-2xl bg-gradient-to-r from-purple-600 to-blue-600 opacity-20 blur transition duration-500 group-hover:opacity-30"
			></div>

			<div
				class="relative rounded-2xl border border-slate-700/50 bg-slate-900/90 p-6 shadow-2xl backdrop-blur-xl sm:p-8"
			>
				{#if successMessage}
					<div
						class="my-6 flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4"
					>
						<div
							class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-500/20"
						>
							<svg
				...
				{#if errorMessage}
					...
					>
						<div
							class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-500/20"
						>
							<svg
					...
					<form onsubmit={handleSubmit} novalidate class="space-y-5">
						...
						<div class="flex items-start gap-4">
							...
							<div class="group/field shrink-0 pt-8">
								<label
									class="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-600/50 bg-slate-800/50 px-4 py-3 transition-all duration-200 hover:border-slate-500/50 hover:bg-slate-800"
									for="is_incognito"
								>
									<input
		...
		<!-- Footer hint (hidden when success message is shown) -->
		{#if !successMessage}
			<p class="mx-8 mt-6 text-center text-sm text-slate-500">
				Клиент должен быть осведомлён о том, что в ближайшее время с ним свяжутся специалисты
				компании Новострой
			</p>
		{/if}
	</div>
